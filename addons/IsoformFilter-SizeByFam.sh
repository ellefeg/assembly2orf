...
join -1 1 -2 2 "$species"_uniqORFs "$fnodes" | sed 's/ /\t/g' | sed 's/[0-9a-zA-Z]*_//' | awk '{print $1, $1, $2}' | sed 's/ /\t/g' | sed 's/|.*\tT/\tT/' | sort -k2,2 > "$species"_familyXref

sed -i 's/\t[0-9a-zA-Z]*_/\t/' "$species"_size

join -1 2 -2 2 "$species"_familyXref "$species"_size | sed 's/ /\t/g' | sed 's/_i[0-9]*\t/\t/g' | awk '{print $2, $3, $1, $4}' | sed 's/ /#/' | sed 's/ /\t/g' | sort -nrk3,3 | sort -u -k1,1 | cut -f 2 | grep "TRINITY" | sort > "$species"_seqOfInterest
...
# Cleanup
# -----------------------------------------
rm "$species"_nameReference $(basename "$fnodes_unsorted")_"$species"_sorted "$species"_size "$species"_uniqORFs "$species"_familyXref "$species"_seqOfInterest "$species"_seqToGet
